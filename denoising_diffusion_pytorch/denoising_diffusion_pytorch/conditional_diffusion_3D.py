...
from Diffusion_motion_field.denoising_diffusion_pytorch.denoising_diffusion_pytorch.attend import Attend
...
#         self.conditional_diffusion_image = diffusion_model.conditional_diffusion_image
#         self.conditional_diffusion_EF = diffusion_model.conditional_diffusion_EF
#         self.conditional_diffusion_seg = diffusion_model.conditional_diffusion_seg
#         print('condition time frame: ', self.conditional_diffusion_timeframe)
#         print('condition image: ', self.conditional_diffusion_image)
#         print('condition EF: ', self.conditional_diffusion_EF)
...
#                     self.opt.zero_grad()
#                     batch_x0_image, batch_condition_tf, batch_condition_tf_normalized, batch_condition_image, batch_condition_EF = batch
#                     data_x0 = batch_x0_image.to(device)
#                     data_condition_tf_normalized = batch_condition_tf_normalized.to(device)
#                     data_condition_image = batch_condition_image.to(device)
#                     data_condition_EF = batch_condition_EF.to(device)

#                     if not self.conditional_diffusion_timeframe:
#                         data_condition_tf_normalized = None
...
#                         loss = self.model(data_x0, condition_timeframe = data_condition_tf_normalized, condition_image = data_condition_image, condition_EF = data_condition_EF)
...
#         self.conditional_diffusion_timeframe = diffusion_model.conditional_diffusion_timeframe
#         self.conditional_diffusion_image = diffusion_model.conditional_diffusion_image
...
#     def sample_3D_w_trained_model(self, trained_model_filename, save_file, image_file, slice_range, input_timeframe = None):
...
#         print('condition image file: ', image_file)
...
#             if self.conditional_diffusion_image:
#                 data_x0, data_timeframe, data_condition_image = datas[0], datas[1], datas[2]
#                 data_condition_image_save = torch.clone(data_condition_image).numpy().squeeze() 
#                 data_condition_image_save = Data_processing.normalize_image(data_condition_image_save, normalize_factor = 'equation', image_max = 1000, image_min = -500, invert = True)
#                 nb.save(nb.Nifti1Image(zoom(data_condition_image_save,(2,2,1),order = 3), affine), os.path.join(os.path.dirname(save_file), 'condition_image.nii.gz'))
#             else:
#                 data_x0, data_timeframe = datas[0], datas[1]
            
#             if input_timeframe is not None:
#                 data_timeframe = torch.from_numpy(input_timeframe).float()

#             # print('data condition shape: ', data_condition_image.shape)
#             print('data_timeframe: ',data_timeframe, data_timeframe.shape)

#             data_timeframe = data_timeframe.to(device)
#             if self.conditional_diffusion_image:
#                 data_condition_image = data_condition_image.to(device)        
#                 pred_mvf = self.ema.ema_model.sample(condition_timeframe = data_timeframe, condition_image = data_condition_image, batch_size = self.batch_size)
#             else:
#                 pred_mvf = self.ema.ema_model.sample(condition_timeframe = data_timeframe, condition_image = None, batch_size = self.batch_size)
                         
#         pred_mvf = pred_mvf.detach().cpu().numpy().squeeze()
#         pred_mvf = Data_processing.normalize_image(pred_mvf, normalize_factor = 'equation', image_max = 20, image_min = -20, invert = True)
#         print(np.max(pred_mvf), np.min(pred_mvf))

#         # save
#         mvf1 = pred_mvf[0:3,...]; mvf1 = np.moveaxis(mvf1, 0, -1); mvf1 = zoom(mvf1, (2,2,1,1), order=1)
#         # mvf2 = pred_mvf[3:6,...]; mvf2 = np.moveaxis(mvf2, 0, -1); mvf2 = zoom(mvf2, (2,2,1,1), order=1)
#         # mvf3 = pred_mvf[6:9,...]; mvf3 = np.moveaxis(mvf3, 0, -1); mvf3 = zoom(mvf3, (2,2,1,1), order=1)
#         # mvf4 = pred_mvf[9:12,...]; mvf4 = np.moveaxis(mvf4, 0, -1); mvf4 = zoom(mvf4, (2,2,1,1), order=1)
#         # nb.save(nb.Nifti1Image(mvf1, affine), os.path.join(os.path.dirname(save_file), 'pred_mvf1.nii.gz'))
#         # nb.save(nb.Nifti1Image(mvf2, affine), os.path.join(os.path.dirname(save_file), 'pred_mvf2.nii.gz'))
#         # nb.save(nb.Nifti1Image(mvf3, affine), os.path.join(os.path.dirname(save_file), 'pred_mvf3.nii.gz'))
#         # nb.save(nb.Nifti1Image(mvf4, affine), os.path.join(os.path.dirname(save_file), 'pred_mvf4.nii.gz'))
#         nb.save(nb.Nifti1Image(mvf1[:,:,:,0], affine), os.path.join(os.path.dirname(save_file), 'pred_mvf_x.nii.gz'))
#         nb.save(nb.Nifti1Image(mvf1[:,:,:,1], affine), os.path.join(os.path.dirname(save_file), 'pred_mvf_y.nii.gz'))
#         nb.save(nb.Nifti1Image(mvf1[:,:,:,2], affine), os.path.join(os.path.dirname(save_file), 'pred_mvf_z.nii.gz')) 
#         print(np.max(mvf1), np.min(mvf1),   np.unravel_index(np.argmax(mvf1), mvf1.shape), np.unravel_index(np.argmin(mvf1), mvf1.shape))
