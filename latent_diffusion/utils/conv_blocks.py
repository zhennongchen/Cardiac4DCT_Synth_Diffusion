...
from Diffusion_motion_field.latent_diffusion.utils.attention_blocks import Attention, zero_module
...
class BasicUp(nn.Module):
    ...
    def forward(self, x, emb=None):
        if self.learnable_interpolation:
            ...
            y = self.upsample(x)
            # new_size = self.calc_shape(x.shape[2:]) 
            return  y#F.interpolate(x, size=new_size, mode='nearest-exact')
