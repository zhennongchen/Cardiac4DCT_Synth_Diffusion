...
import Diffusion_motion_field.Discriminator.model as discriminator
import Diffusion_motion_field.Generator as Generator
main_path = '/mnt/camca_NAS/4DCT'

#######################
trial_name = 'MVF_EDM_down_10tf_imgcon_EFcon_warp_adversarial'
latent = True if 'latent' in trial_name else False 
EF_loss_weight = 0
adversarial_loss_weight = 0.1 if 'adversarial' in trial_name else 0
...
pre_trained_model = os.path.join('/mnt/camca_NAS/4DCT','models', 'MVF_EDM_down_10tf_imgcon_EFcon_warp', 'models/model-1000.pt')
start_step = 0
...
conditional_diffusion_seg = True if 'warp' in trial_name else False
...
patient_class_val_list  , patient_id_val_list = patient_class_train_list, patient_id_train_list
...
generator_train = Generator.Dataset_dual_3D(
    ...
    condition_on_seg = conditional_diffusion_seg,
    mvf_cutoff = [-20,20],
    latent_cutoff = [-30,30],
    shuffle = True,
    augment = True,
...
elif 'warp' in trial_name and 'adversarial' not in trial_name:
...
elif 'warp' in trial_name and 'adversarial' in trial_name:
    D_model = discriminator.CNN_EF_predictor_temporalConv(init_dim = 16,channels = 1,dim_mults = (2,4,8),full_attn = (None,None, None),act = 'LeakyReLU',)
    trainer = edm_adversarial.Trainer(diffusion_model= diffusion_model, discriminator_model = D_model, generator_train = generator_train, generator_val = generator_val,
                                      EF_loss_weight = EF_loss_weight, adversarial_loss_weight = adversarial_loss_weight,
...
trainer.save_models_every = 1
trainer.validation_every = 1
