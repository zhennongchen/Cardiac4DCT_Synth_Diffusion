...
class GaussianDiffusion3D(nn.Module):
    ...
    def ddim_sample(self, shape, condition = None, return_all_timesteps = False, save_all_timesteps_ref = None):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            # coefficients.append([time, time_next, alpha_next.sqrt(), c, sigma])
            # print('time: ', time, 'time_next: ', time_next, 'alpha_next: ', alpha_next.sqrt(), 'c: ', c, 'sigma: ', sigma)
        ...
        # df = pd.DataFrame(coefficients, columns = ['time', 'time_next', 'alpha_next', 'c', 'sigma'])
        # df.to_excel(os.path.join('/mnt/mount_zc_NAS/diffusion_ct_motion/models/3D_sanity_check/pred_images','DDIM-eta'+suffix,'coefficients.xlsx'))
        return ret
...
class Sampler(object):
    ...
    def sample_3D_slice_stack_w_trained_model(self, trained_model_filename, save_file, image_file, slice_range):
        ...
        downsampled_img_info = zoom(img_info, scale_factors, order=3)[:,:,slice_range[0]:slice_range[1]]
