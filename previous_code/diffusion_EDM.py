...
from Diffusion_motion_field.denoising_diffusion_pytorch.denoising_diffusion_pytorch.diffusion_3D import *
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        print('CCCCCConditional diffusion in trainer is : ',self.conditional_diffusion)
...
class Sampler(object):
    ...
    def sample_3D_w_trained_model(self, trained_model_filename,  save_file, image_file, slice_range):
        ...
        with torch.inference_mode():
            if self.conditional_diffusion:
                ...
                data_condition_image_save = torch.clone(data_condition).numpy().squeeze() 
                data_condition_image_save = Data_processing.normalize_image(data_condition_image_save, normalize_factor = 'equation', image_max = np.max(downsampled_img_info), image_min = np.min(downsampled_img_info), invert = True)
                nb.save(nb.Nifti1Image(data_condition_image_save, affine), os.path.join(os.path.dirname(save_file), 'condition_image.nii.gz'))
                
                data_condition = data_condition.to(device)  
                pred_mvf = self.ema.ema_model.sample(condition = data_condition, batch_size = self.batch_size)
            else:
                pred_mvf = self.ema.ema_model.sample(batch_size = self.batch_size)
        ...
        gt = os.path.join('/mnt/camca_NAS/4DCT','mvf_warp0_onecase','Normal', 'CVC1803280940', 'voxel_final/4.nii.gz')
        gt = nb.load(gt).get_fdata()
        nb.save(nb.Nifti1Image(gt[:,:,:,0], affine), os.path.join(os.path.dirname(save_file), 'gt_x.nii.gz'))
        nb.save(nb.Nifti1Image(gt[:,:,:,1], affine), os.path.join(os.path.dirname(save_file), 'gt_y.nii.gz'))
        nb.save(nb.Nifti1Image(gt[:,:,:,2], affine), os.path.join(os.path.dirname(save_file), 'gt_z.nii.gz'))

        # pred_mvf = zoom(pred_mvf, (2,2,1), order=3)
        # nb.save(nb.Nifti1Image(pred_mvf, affine), os.path.join(os.path.dirname(save_file), 'pred_mvf_x.nii.gz'))
