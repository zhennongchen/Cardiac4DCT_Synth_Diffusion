...
embed_channels = 3 if '3' in trial_name else 4
epoch = 100
trained_model_filename = os.path.join(main_path, 'models', trial_name, 'models_mgh/model-' + str(epoch)+ '.pt')
save_folder = os.path.join(main_path, 'models', trial_name, 'pred_mvf')
# save_folder = os.path.join(main_path,'models/MVF_EDM_latent_5tf_imgcon_EFcon_trial2/pred_mvf')
os.makedirs(save_folder, exist_ok=True)

slice_range = [0,96]
image_size_3D = [160,160,slice_range[1]-slice_range[0]]

# picked_tf = [0.1,0.3,0.5,0.7,0.9]
...
model = VAE_model.VAE(
        ...
        hid_chs =   [64,128,256] if embed_channels == 3 else [64,128,256,512],
        kernel_sizes=[3,3,3] if embed_channels == 3 else [3,3,3,3],
        strides =    [1,2,2] if embed_channels == 3 else [1,2,2,2],)

for i in range(0,len(patient_id_list)//2):
    ...
    # save_folder_case_list = ff.find_all_target_files(['epoch1580*'],os.path.join(save_folder,patient_class, patient_id))
    save_folder_case = os.path.join(save_folder,patient_class, patient_id,'epoch'+str(epoch))
    ff.make_folder([os.path.join(save_folder, patient_class),os.path.join(save_folder, patient_class, patient_id),save_folder_case])
    ...
    tf_choices = np.arange(0,tf_num,1)

    for save_folder_case in [save_folder_case]:
        ...
        for tf_choice in tf_choices:
            if os.path.isfile(os.path.join(save_folder_case, 'pred_latent_tf'+str(tf_choice)+'.nii.gz'))==0:
                ...
                all_mae,x_mae, y_mae,z_mae = sampler.sample_3D_w_trained_model(trained_model_filename=trained_model_filename,affine = affine, save_file = save_file, 
                                                                               save_latent = True, save_decode = False, only_do_decode = False, save_gt = False)
